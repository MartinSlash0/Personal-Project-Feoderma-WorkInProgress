...
document.addEventListener('click', function(e){
    if(e.target.id === 'load-more-btn'){
        if(document.getElementById('load-more-btn').textContent === 'Покажи стари статии'){
            ...
            document.getElementById('load-more-btn').textContent = 'Скрий стари статии'
        }
        else if(document.getElementById('load-more-btn').textContent === 'Скрий стари статии'){
            document.getElementById('older-articles-container').classList.toggle('articles-container')
            document.getElementById('older-articles-container').classList.toggle('display')
            document.getElementById('load-more-btn').textContent = 'Покажи стари статии'
        } 
    }
    ...
    articlesArray.forEach(article => {
        ...
        <div class="article">
                        <div class="article-img">
                            <a href="${article.link.bg}" target="_blank">
                                <img src="${article.img}" alt="${article.alt.bg}">
                            </a>
                        </div>
                        <div class="article-text">
                            <p class="date">${article.date.bg}</p>
                            <h2>${article.brand}</h2>
                            <p>${article.info.bg}</p>
                            <div class="btn-container">
                                <a class="article-link" href="${article.link.bg}" target="_blank" data-read="${article.uuid}">
                                    Виж повече...
                                </a>
                            </div>
                        </div>
